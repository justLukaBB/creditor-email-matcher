...
Phase 3 extracts ONLY these fields:
- `gesamtforderung` (total claim amount)
- `client_name`
- `creditor_name`

Extended roadmap fields (Forderungsaufschluesselung breakdown, Bankdaten, Ratenzahlung) are NOT extracted in Phase 3 - deferred to Phase 4.
...
3. `SourceExtractionResult` - Result from a single source (one attachment or email body):
   ...
   - `components: Optional[dict]` - {"hauptforderung": float, "zinsen": float, "kosten": float}
     **IMPORTANT:** The `components` dict is ONLY used internally to compute Gesamtforderung
     when no explicit total is labeled. It stores intermediate values for the sum calculation:
     Gesamtforderung = Hauptforderung + Zinsen + Kosten.
     This is NOT for extracting extended roadmap fields - those are deferred to Phase 4.
...
- The `components` field exists solely to support the computation logic (sum to get Gesamtforderung)
  </action>
  <verify>
```bash
