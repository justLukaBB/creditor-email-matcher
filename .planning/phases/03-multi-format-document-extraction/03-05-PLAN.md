...
must_haves:
  ...
  key_links:
    ...
    - from: "app/services/extraction/image_extractor.py"
      ...
      via: "os.unlink for resized images"
      pattern: "os\\.unlink"
---
...
def extract(self, image_path: str) -> SourceExtractionResult:
    ...
    try:
        ...
        # IMPORTANT: Clean up resized temp file if we created one
...
def _resize_image(self, image_path: str) -> str:
    """Resize large images to reduce token usage. Returns path to temp file."""
    from PIL import Image
    import tempfile
    import os
...
- **CRITICAL:** Clean up resized temp files in finally block to handle exceptions
- Track `resized_path` separately from `working_path` to know if cleanup is needed
- Use `os.unlink(resized_path)` for cleanup, wrapped in try/except for best-effort
  </action>
  <verify>
```bash
...
```
  ...
  <done>ImageExtractor uses Claude Vision for JPG/PNG, resizes large images, checks token budget before API calls, cleans up temp files in finally block</done>
</task>
...
3. Resized temp files cleaned up in finally block
4. Consolidator applies highest-amount-wins correctly
5. Consolidator defaults to 100 EUR when no amounts
6. All extractors exported from __init__.py
</verification>

<success_criteria>
- Images extract via Claude Vision with token budget check
- Large images resized before API call
- Temp files from resize operation cleaned up after API call
