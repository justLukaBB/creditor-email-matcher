...
3. Implement _correct_ocr_errors(text: str) -> str:
   ...
   - For each word, try Umlaut substitutions (ii->ue, oe->o-umlaut, ae->a-umlaut)
...
4. Add OCR substitution map:
   ```python
   umlaut_substitutions = {
       'ii': 'u',  # Will become u-umlaut after checking
       'oe': 'o',  # Will become o-umlaut after checking
       'ae': 'a',  # Will become a-umlaut after checking
   }
   ```
   Note: The substitution replaces digraph with base letter, then we try adding umlaut variant
...
Use structlog for logging, matching project patterns.
  ...
  <verify>
python -c "from app.services.extraction.german_preprocessor import GermanTextPreprocessor; p = GermanTextPreprocessor(); print(p.preprocess('Muller'))"
  </verify>
  <done>
GermanTextPreprocessor class exists with preprocess() method. Unicode normalization works. OCR correction is conservative (dictionary-validated). pyspellchecker added to requirements.txt.
  </done>
</task>
...
1. Test GermanTextPreprocessor:
   ...
   class TestGermanTextPreprocessor:
       def test_unicode_normalization_composed_unchanged(self):
           ...
           assert preprocessor.preprocess("Mueller") == "Mueller"
       ...
       def test_ocr_correction_muller_to_mueller(self):
           # Only if Mueller is in dictionary
           result = preprocessor.preprocess("Muller")
           # Result may or may not be corrected depending on dictionary
           assert result in ["Muller", "Mueller"]

       def test_ocr_correction_feuer_unchanged(self):
           # Legitimate word should NOT become "Feuer"
...
<success_criteria>
- GermanTextPreprocessor.preprocess() normalizes Unicode and optionally corrects OCR errors
