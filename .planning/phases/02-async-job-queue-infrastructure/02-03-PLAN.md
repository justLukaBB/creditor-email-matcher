...
must_haves:
  truths:
    ...
    - "gc.collect() runs after every processed job for memory stability"
  artifacts:
...
1. **Retry predicate function** `should_retry(retries_so_far, exception)`:
...
2. **Actor definition** `@dramatiq.actor(max_retries=5, min_backoff=15000, max_backoff=300000, retry_when=should_retry, queue_name="email_processing")`:

3. **process_email(email_id: int)** function body:
   - Import SessionLocal from app.database (lazy import to avoid circular deps)
   ...
   **Exception handling:**
   - In except block: load email fresh, set processing_status = "failed", processing_error = str(e), increment retry_count, commit
   - Re-raise the exception so Dramatiq retry logic kicks in
...
IMPORTANT: This actor extracts the processing logic currently in webhook.py's `process_incoming_email()` function. The imports for email_parser, entity_extractor, entity_extractor_claude, mongodb_service, email_notifier, DualDatabaseWriter, IdempotencyService, generate_idempotency_key should all be lazy (inside the function) to avoid import-time side effects in worker processes.

4. **Update app/actors/__init__.py**: Uncomment/add the actor import line:
   ```python
...
Run `python -c "from app.actors.email_processor import should_retry; print(should_retry(0, ValueError()))"` -- should print False.
  ...
  <done>
Email processor actor exists at app/actors/email_processor.py with: Dramatiq actor decorator (max_retries=5, exponential backoff 15s-5min), selective retry predicate, FOR UPDATE SKIP LOCKED row locking, full state machine transitions, DualDatabaseWriter integration, gc.collect() memory management. Actor registered in app/actors/__init__.py.
  </done>
</task>
...
- gc.collect() in actor: memory management for 512MB constraint
</verification>
