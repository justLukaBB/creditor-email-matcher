...
must_haves:
  truths:
    ...
    - "Failure notifier sends email only on permanent job failure (after all retries exhausted)"
  ...
  key_links:
    ...
    - from: "app/actors/email_processor.py"
      ...
      via: "on_failure callback imports and calls notify_permanent_failure"
...
Service to send email notifications when jobs permanently fail (per CONTEXT.md: "Email notification on permanent failure after all retries exhausted").
...
3. `notify_permanent_failure(email_id: int)` standalone function:
   - This function is called by the actor's on_failure callback (defined in Plan 02-03, app/actors/email_processor.py's on_process_email_failure function)
   - Load email from database to get from_email, subject, and processing_error
   - Call failure_notifier.send_failure_notification(...)
   - Wrap in try/except to never raise (notification is best-effort)
   - Log with structlog: `logger.info("permanent_failure_notification_sent", email_id=email_id)` on success, `logger.error("permanent_failure_notification_failed", ...)` on failure
  </action>
  <verify>
...
Run `python -c "from app.services.failure_notifier import notify_permanent_failure; print(callable(notify_permanent_failure))"` -- should print True.
  </verify>
  <done>
Job status API at /api/v1/jobs with list, detail, and manual retry endpoints. Failure notifier sends email alerts on permanent failures using existing SMTP config. notify_permanent_failure function is importable and callable by the actor's on_failure callback (Plan 02-03). Both handle missing configuration gracefully.
  </done>
</task>
...
- notify_permanent_failure is importable for actor's on_failure callback
...
- Email notification sent on permanent failure (when SMTP configured) -- triggered by actor's on_failure callback, not on every retry
